...
public class StickyItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int m = 0, size = parent.getChildCount(); m < size; m++) {
            ...
            if (mStickyView.isStickyView(view)) {
                ...
                if (mStickyItemViewMarginTop != 0 && mStickyPositionList.size() == 1) {
                    return;
                }
     ...
     * @param list
     */
    public void setCacheStickyViewList(List<Integer> list) {
        mStickyPositionList.clear();
        mStickyPositionList.addAll(list);
    }
        ...
        try {
            ...
            if (!mStickyPositionList.contains(position)) {
                if (mStickyPositionList.size() == 0) {
                    ...
                    int addPos = 0;
                    for (int i = 0; i < mStickyPositionList.size(); i++) {
                        if (position > mStickyPositionList.get(i)) {
                            addPos = i;
                        } else {
                            break;
                        }
                    }
                    mStickyPositionList.add(addPos + 1, position);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
